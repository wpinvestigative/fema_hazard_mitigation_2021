...
Of the $11 billion FEMA has allocated for this program over the past decade, only $1.5 billion has been spent.
...
mega_df %>%
  ...
  summarize(average_disaster_count=mean(incident_count, na.rm=T)) %>%
...
county_projects_disasters %>% 
  mutate(ratio=projects/disasters) %>% 
  group_by(ur) %>% 
  summarize(average_ratio = mean(ratio, na.rm=T)) %>% 
  kable()
```

Half of the rural communities that fell in a disaster declared zone aren't getting any Hazard Mitigation Project money.
